...
outbreak_date <- read.csv("Data/outbreak date_2022_2.csv",encoding = "UTF-8",header=T,sep=",")
...
### Rt estimation for each city with outbreak in Omicron wave2
...
for (i in 1:length(citylist)){
  ...
  data_input$date <- as.Date(data_input$date,format='%Y-%m-%d',origin="1970-01-01")
  ...
  if (end_date > as.Date("2022-11-27")){
  ...
  # city_folder <- paste0('Result/Rt of city/2022_2/',city_name)
  # dir.create(city_folder)
  # save.image(file =paste0('Result/Rt of city/2022_2/',city_name,'/',city_name,'.RData'))
  # write.table(out$R,paste0('Result/Rt of city/2022_2/',city_name,'/',city_name,'.csv'),sep=",",row.names=FALSE)
  # write.table(d1,paste0('Result/Rt of city/2022_2/',city_name,'/',city_name,'_input.csv'),sep=",",row.names=FALSE)
